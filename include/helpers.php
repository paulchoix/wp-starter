...
function excerpt( string $string, int $length = 470, string $suffix = '...' )
{
    ...
    $tags_appended = array_map( function( $tag ) { return $tag . ' '; }, $tags_to_append );
...
{
    $array = preg_split("/\r\n|\n|\r/", esc_html( $string ) );
    $array = array_filter( $array, function( $a ) { if( $a ) return $a; } );
...
function mailto( string $email = '', string $subject = '', string $body = '', $cc = null, $bcc = null )
{
    ...
    if( $cc )
    {
        ...
        if( is_array( $cc ) )
        {
            $cc = array_filter( $cc, function( $c ) { if( is_email( $c ) ) return $c; } );
    ...
    if( $bcc )
    {
        ...
        if( is_array( $bcc ) )
        {
            $bcc = array_filter( $bcc, function( $c ) { if( is_email( $c ) ) return $c; } );
    ...
    if( $output_array )
    {
        $joined = array_map( function( $key, $value ) { return $key . '=' . $value; }, array_keys( $output_array ), $output_array );
